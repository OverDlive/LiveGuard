...
public class SportOnboardingAdapter extends RecyclerView.Adapter<SportOnboardingAdapter.SportViewHolder> {
    ...
    public SportOnboardingAdapter(Context context, List<Integer> pageLayouts, ChoiceListener listener) {
            ...
            if (position == 0) {
                ...
                if (startButton != null) {
                    startButton.setOnClickListener(v -> {
                        ((SportOnboardingActivity) context).moveToNextPage();
                    });
                }
            } else {
                for (int i = 1; i <= 4; i++) {
                    ...
                    if (choiceButton != null) {
                        final int finalPosition = position; // position을 final로 고정
